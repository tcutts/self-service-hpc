...
For local python development, always use a virtual environment, not the system python.  The venv is located in the project root at `.venv/` — the python binary is `.venv/bin/python3` (i.e. `<workspace_root>/.venv/bin/python3`).  Always invoke it from the project root or use an absolute/relative path that resolves to the project root `.venv`.
!------------------------------------------------------------------------------------
